...
def load_inputs():
    ...
    try:
        ...
        recliner2gcbm_config_dir = cPickle.load(
            open(r'inputs\recliner2gcbm_config_dir.pkl')
        )
        recliner2gcbm_output_path = cPickle.load(
            open(r'inputs\recliner2gcbm_output_path.pkl')
        )
        recliner2gcbm_exe_path = cPickle.load(
            open(r'inputs\recliner2gcbm_exe_path.pkl')
        )
...
def save_objects():
    try:
        ...
        raise IOError("Failed to save objects.")
...
def load_objects():
    ...
    try:
        ...
        raise IOError("Failed to load objects.")
...
if __name__ == "__main__":
    ...
    #load_inputs()
    db_url = r'postgresql://postgres:postgres@192.168.4.1:5432/princegeorge'
    db = pgdata.connect(
         db_url,
    ...
    intersect = rollback.intersect_disturbances_inventory.IntersectDisturbancesInventory(
        inventory, spatialBoundaries, rollback_range, PP
    )
    calcDistDEdiff = rollback.update_inventory.CalculateDistDEdifference(inventory, PP)
    calcNewDistYr = rollback.update_inventory.CalculateNewDistYr(
        inventory, rollback_range, historicHarvest.getYearField(), PP
    )
    updateInv = rollback.update_inventory.updateInvRollback(
        inventory,
        inventory_raster_out,
        rollbackDisturbances,
        rollback_range,
        resolution,
        tiler_scenarios.itervalues().next()[0],
        PP,
    )
    ...
    r2GCBM = recliner2GCBM.recliner2GCBM.Recliner2GCBM(
        config_dir=recliner2gcbm_config_dir,
        output_path=recliner2gcbm_output_path,
        transitionRules=transitionRules,
        yieldTable=yieldTable,
        aidb=AIDB,
        ProgressPrinter=PP,
        exe_path=recliner2gcbm_exe_path,
    )
    gcbmConfigurer = GCBMconfig.configure_gcbm.ConfigureGCBM(
        output_dir=gcbm_raw_output_dir,
        gcbm_configs_dir=gcbm_configs_dir,
        GCBM_scenarios=GCBM_scenarios,
        base_scenario='Base',
        inventory=inventory,
        reportingIndicators=reportingIndicators,
        resolution=resolution,
        rollback_range=rollback_range,
        actv_start_year=activity_start_year,
        future_range=future_range,
        ProgressPrinter=PP,
    )
    ### Execute Functions
    ...
    for base_scenario in [
        scen for scen in tiler_scenarios if scen.lower() == 'base'
    ]:  # ***
        tiler.processProjectedDisturbances(
            base_scenario, tiler_scenarios[base_scenario]
        )
    ...
    for miti_scenario in [scen for scen in tiler_scenarios if scen.lower() != 'base']:
        tiler.processProjectedDisturbances(
            miti_scenario, tiler_scenarios[miti_scenario]
        )
